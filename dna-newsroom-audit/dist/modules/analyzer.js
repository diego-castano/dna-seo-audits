...
exports.analyzeRobotsTxt = analyzeRobotsTxt;
exports.analyzeSitemap = analyzeSitemap;
exports.detectCatchAll = detectCatchAll;
exports.analyzeLinkability = analyzeLinkability;
exports.analyzePagination = analyzePagination;
exports.analyzeListingRoute = analyzeListingRoute;
exports.analyzeListingRoutes = analyzeListingRoutes;
exports.analyzeUrl = analyzeUrl;
exports.detectSpaIssues = detectSpaIssues;
exports.analyzeStructuredData = analyzeStructuredData;
exports.analyzeCanonicalRatio = analyzeCanonicalRatio;
const parser_1 = require("./parser");
const url_1 = require("../utils/url");
// ============================================
// ROBOTS.TXT ANALYZER
// ============================================
function analyzeRobotsTxt(result) {
    const findings = [];
    const signature = (0, parser_1.detectBodySignature)(result.body);
    ...
    if (result.status !== 200) {
        findings.push({
            ...
            whatCrawlersSee: (0, parser_1.getEvidenceSnippet)(result.body),
    ...
    return {
        ...
        evidenceSnippet: (0, parser_1.getEvidenceSnippet)(result.body),
        findings
    };
}
// ============================================
// SITEMAP ANALYZER
// ============================================
function analyzeSitemap(results) {
    const findings = [];
    const variants = [];
    ...
    for (const result of results) {
        ...
        if (result.status === 200 && signature.isXML) {
            ...
            const parsed = (0, parser_1.parseXMLSitemap)(result.body);
...
// ============================================
function detectCatchAll(robotsResult, sitemapResult, homeResult) {
    const findings = [];
    const signals = [];
    const homeHash = (0, parser_1.computeHash)((0, parser_1.normalizeHTML)(homeResult.body));
    const robotsHash = (0, parser_1.computeHash)((0, parser_1.normalizeHTML)(robotsResult.body));
    const sitemapHash = sitemapResult ? (0, parser_1.computeHash)((0, parser_1.normalizeHTML)(sitemapResult.body)) : '';
    ...
    const robotsSimilarity = (0, parser_1.calculateSimilarity)(robotsResult.body, homeResult.body, robotsHash, homeHash);
    const sitemapSimilarity = sitemapResult ? (0, parser_1.calculateSimilarity)(sitemapResult.body, homeResult.body, sitemapHash, homeHash) : { isExactMatch: false, isHighSimilarity: false, score: 0, reasons: [] };
    ...
    if (detected) {
        findings.push({
            ...
            affectedUrls: [robotsResult.url, sitemapResult?.url].filter(Boolean)
        });
    }
    return {
        detected,
...
// ============================================
function analyzeLinkability(parsed, pageType = 'home') {
    const findings = [];
    ...
    const nonCrawlableIssues = (0, parser_1.detectNonCrawlableNavigation)(anchors);
...
// PAGINATION ANALYZER
// ============================================
function analyzePagination(html, anchors) {
    const findings = [];
    const patterns = [];
...
// ============================================
function analyzeListingRoute(result, baseUrl) {
    ...
    let isShell = false;
    if (exists) {
        const parsed = (0, parser_1.parseHTML)(result.body, baseUrl);
        title = parsed.title;
...
function analyzeListingRoutes(routes) {
    const findings = [];
...
// URL ANALYSIS
// ============================================
function analyzeUrl(result, baseUrl, homeUrl) {
    const findings = [];
    const parsed = (0, parser_1.parseHTML)(result.body, baseUrl);
    const signature = (0, parser_1.detectBodySignature)(result.body);
    // Canonical analysis
    ...
    if (parsed.canonical === '' && !(0, url_1.isHomepage)(result.url)) {
        findings.push({
    ...
    if (parsed.title === '') {
        findings.push({
            ...
            whatCrawlersSee: (0, parser_1.getEvidenceSnippet)(result.body, 5),
            affectedUrls: [result.url]
        });
    }
...
// SPA DETECTION
// ============================================
function detectSpaIssues(urlAnalyses) {
    const findings = [];
    const shellClusters = [];
    const softFourOhFours = [];
    // Group by hash
    ...
    for (const analysis of urlAnalyses) {
        if (analysis.status === 200) {
            const signals = [];
            if (analysis.title === '')
                signals.push('empty title');
            if (analysis.ogTags.title === '' && analysis.ogTags.description === '')
                signals.push('empty OG');
            if (analysis.bodyTextLength < 500)
                signals.push(`low text (${analysis.bodyTextLength} chars)`);
            if (analysis.shellClusterId)
                signals.push(`shell cluster #${analysis.shellClusterId}`);
            if (signals.length >= 2) {
...
// ============================================
function analyzeStructuredData(urlAnalyses, articleUrls, homeUrl) {
    const findings = [];
    ...
    const postsWithoutArticleSchema = [];
    for (const url of articleUrls) {
        const analysis = urlAnalyses.find(a => a.url === url);
        if (analysis) {
            const hasArticleSchema = analysis.jsonLdTypes.some(t => t === 'Article' || t === 'NewsArticle' || t === 'BlogPosting');
            if (!hasArticleSchema) {
...
// ============================================
function analyzeCanonicalRatio(urlAnalyses) {
    const findings = [];
    const nonHomePages = urlAnalyses.filter(a => !(0, url_1.isHomepage)(a.url));
