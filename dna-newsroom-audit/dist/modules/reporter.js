...
function generateBullets(result) {
    const bullets = [];
...
function generateDomainHTML(result, isActive) {
            ...
            <tbody>
              ${Object.entries(result.inventory.counts).filter(([_, count]) => count > 0).map(([type, count]) => `
                <tr>
                  <td>${type}</td>
                  <td>${count}</td>
                  <td>${result.inventory.byType[type]?.filter(p => p.needsEnhancement).length || 0}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
...
// FILE WRITING
// ============================================
function writeReports(results, outputDir, formats) {
